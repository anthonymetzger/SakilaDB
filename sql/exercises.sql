...
select title, description, rating, rental_duration from film WHERE rental_duration >= 3;
...
SELECT * FROM customer WHERE active = 0 AND first_name LIKE 'C%' OR first_name LIKE 'S%' OR first_name LIKE 'T%';
...
SELECT title, description, release_year FROM film WHERE BETWEEN LENGTH(description) < 120 AND LENGTH(description) > 100;
...
SELECT title, description, length, rental_rate FROM film WHERE description LIKE "%Database%" AND length > 180;
...
SELECT payment_id, payment_date, amount FROM payment WHERE amount > 5 LIMIT 50 OFFSET 50;

# 7c. Select all columns from the customer table, limiting results to those where the zero-based index is between 101-200.
SELECT * FROM customer WHERE customer_id LIMIT 100 OFFSET 100;
...
SELECT 
  ...
  customer.last_name  AS customer_last_name,
  actor.first_name    AS actor_first_name,
  actor.last_name     AS actor_last_name FROM customer INNER JOIN actor ON customer.last_name = actor.last_name;
...
SELECT 
  ...
  city.city        AS city_name,
  address.district,
  address.postal_code FROM staff
LEFT JOIN address
